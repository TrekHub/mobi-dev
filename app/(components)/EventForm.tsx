...
const EventForm = ({ event }: any) => {
    ...
    const [fileName, setFileName] = useState<string>("" || event.banner_img);

    const handleChangFile = (e: ChangeEvent<HTMLInputElement>) => {
        console.log("clicked");
        if (e.target.files) {
                ...
                <form method="post" onSubmit={handleSubmit}>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex  flex-col items-start justify-start w-full">
                            ...
                            <button type='button' className='w-full' onClick={handleBtnClick}>
                                {/* <input id="dropzone-file" type="file" onChange={handleChangFile} className="hidden" /> */}
                                <label htmlFor="banner_img" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    ...
                                    <input id="dropzone-file" ref={fileRef} type="file" onChange={({ target: { files } }) => {
                                    ...
                                    {file ? <img src={URL.createObjectURL(file)} width={60}  height={60} alt="" /> : <p className="text-xs text-gray-500 dark:text-gray-400">{fileName}</p> }
